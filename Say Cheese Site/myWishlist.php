        ...
        <td>
          <div class="container">
            <table border="0">
				...
				$outersql="SELECT `productID` FROM `userwishlist` WHERE `username` = '".$_SESSION["userName"]."'; ";
				...
				if (mysqli_num_rows($outerresult)>0)
				{
					while($outerrow = mysqli_fetch_assoc($outerresult))
					{
						...
						if (mysqli_num_rows($innerresult)>0)
						{
							...
							{
								echo "<tr>
								<td class='tdmiddle'><div class='imgcontainer'><img src='".$innerrow["imgPath"]."' width='150' height='150' ></div></td><td width='20'></td>
								<td width='200'>".$innerrow["title"]."</td>
								<td><a href='deleteFromWishlist.php?id=".$innerrow["productID"]."'><i class='fa-solid fa-x'></i> </a></td>
								</tr>
					...
					echo "Seems like nothing on your Wish list. Why wait? Add some Now";
				}
